 ...
 * come from src/data/question-counts.json, generated by
 * scripts/sync-question-counts.js and pinned by tests/question_bank_test.js.
 *
 * They used to be hardcoded and rotted badly: the footer still advertised
 * "482+ soru", showed seven categories as "YAKINDA" with 5 questions each, and
 * left Philosophy out entirely, while the bank held 2400 questions across 12
 * finished categories. Visitors were being told the game was half-built.
 */
const CATEGORY_DISPLAY: Array<{ key: keyof typeof questionCounts.categories; name: string; emoji: string }> = [
  { key: 'general',    name: 'Genel Kültür',  emoji: '🧠' },
  { key: 'cinema',     name: 'Sinema & Dizi', emoji: '🎬' },
  { key: 'games',      name: 'Oyunlar',       emoji: '🎮' },
  { key: 'sports',     name: 'Spor',          emoji: '⚽' },
  { key: 'history',    name: 'Tarih',         emoji: '📜' },
  { key: 'music',      name: 'Müzik',         emoji: '🎵' },
  { key: 'anime',      name: 'Anime',         emoji: '🎌' },
  { key: 'technology', name: 'Teknoloji',     emoji: '💻' },
  { key: 'literature', name: 'Edebiyat',      emoji: '📚' },
  { key: 'math',       name: 'Matematik',     emoji: '🔢' },
  { key: 'geography',  name: 'Coğrafya',      emoji: '🌍' },
  { key: 'philosophy', name: 'Felsefe',       emoji: '🤔' },
];
    ...
    content: (
      <div className="space-y-3 text-sm">
        {CATEGORY_DISPLAY.map((cat) => (
          <div key={cat.key} className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-white/5 border border-white/10">
            <div className="flex items-center gap-2">
              <span className="text-lg">{cat.emoji}</span>
              <span className="text-white font-medium">{cat.name}</span>
            </div>
            <span className="text-gray-400 text-xs font-mono">
              {questionCounts.categories[cat.key]} soru
            </span>
          </div>
        ))}
        <p className="text-gray-500 text-xs pt-1">
          Toplam {questionCounts.total.toLocaleString('tr-TR')} soru, {questionCounts.categoryCount} kategori.
        </p>
      </div>
    ),
  },
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              >{questionCounts.total.toLocaleString('tr-TR')} soru</span>
              <span
                className="text-[11px] px-2.5 py-1 rounded-full"
                style={{
                  color: 'var(--footer-muted)',
